...
%
% Thanks to Todd F Velde (TFV) for help with missing superclasses
...
%--------------------------------------------------------------------------    
    ...
    methods
        ...
        function setExtraParentsIDs(this)
            ...
            for iClass = 1:this.nClasses
                ...
                if(~isempty(extraParents))
                    ...
                    EPIds = zeros(nEP,1);
                    for iEP = 1:nEP
                        parentPos = find(strcmp(this.classNameList,extraParents{iEP}));
        ...
        function setLevelInfo(this)
            ...
            % deal with parents not in the tree (thanks to TFV)
            % would be easier if this was rewritten as a proper tree
            for iClass = 1:this.nClasses
                parentId = find(strcmp(this.classNameList,this.classList(iClass).parent));
                if isempty(parentId)
                    this.classList(iClass).parentId = 0;
                else
                    this.classList(iClass).parentId = parentId;
                end
            end
            % recalculate levels from parents down
            [~,sortedorder] = sort([this.classList.parentId]);
            for iClass = sortedorder
                if this.classList(iClass).parentId > 0
                    this.classList(iClass).level = this.classList(this.classList(iClass).parentId).level + 1;
                else
                    this.classList(iClass).level = 1;
                end
            end 
        ...
        function makePS(this)
            ...
            % Do this after determining positions of all classes as you
            % may have a parent in the same level
            for iClass = 1:this.nClasses
                ...
                if(~isempty(this.classList(iClass).extraParentsIds))
                    ...
                    for iEP = 1:length(EPIds)
                        ...
                        if(all(isfinite(arrowX)) && all(isfinite(arrowY)))
                            fprintf(fid,  this.makeArrowPS(arrowX,arrowY));
                        end
                    end
                end
